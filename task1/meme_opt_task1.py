...
def create_signature(rows, hash_num, hasher):
    columns_signature = []
    for hash_index in range(1,hash_num+1):
...
def is_uniq_pair(hastable,id1,id2):
    key = (id1,id2)
    try:
        exist = hastable[key]
        return False
...
if __name__ == '__main__':
    ...
    similars = set(bands_distribution.flatMap(lambda x: [(*sorted((id1,id2)),JaccardSim(id1,id2,sparse_matrix)) \
                                        for id1,id2 in combinations(x[1],2) if is_uniq_pair(uniq_pairs,id1,id2) and JaccardSim(id1,id2,sparse_matrix)>=0.05]).collect())
