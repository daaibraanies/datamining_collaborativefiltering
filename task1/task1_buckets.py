...
if __name__ == '__main__':
    ...
    bands_distribution = signature_matrix.flatMap(lambda x: [(band_hash, x[0]) for band_hash in map_bands(x[1], bands, bands_r)])\
                                         .groupByKey()\
                                         .filter(lambda x: len(x[1]) > 1)\
                                         .map(lambda x: (sorted(x[1]),1))

    candidate_pairs = bands_distribution.flatMap(lambda x: [(id1,id2) for id1,id2 in combinations(x[0],2)]).partitionBy(30)

    print("{} sec".format(time.time()-local_time))

    sparse_matrix = sparse_matrix.collect()
    sparse_matrix = {key:value for key,value in sparse_matrix}

    similars = candidate_pairs.map(lambda x: (x[0],x[1],JaccardSim(x[0],x[1],sparse_matrix)) if JaccardSim(x[0],x[1],sparse_matrix) >=0.05 else None)\
                              .collect()
