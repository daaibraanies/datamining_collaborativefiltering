...
def get_users_businesses_wordmap(model):
    with open(model, 'r',encoding='utf-8') as outfile:
        ...
        businesses = json.loads(data[1])
        users = json.loads(data[2])
        urev = json.loads(data[3])

    businesses = dict(businesses['bprofile'])
    users = dict(users['uprofile'])
    wordmap = dict(words['word_index'])
    urev = dict(urev['ureviews'])
    return businesses,users,urev,wordmap
...
def is_uniq_pair(hastable,user_id,business_id):
    key = tuple(sorted([user_id,business_id]))
    try:
        exist = hastable[key]
        return False
    except KeyError:
...
def cosineSim(user_id,business_id,profiles):
    user_vector = profiles[user_id]
    business_vector = profiles[business_id]
...
if __name__ == '__main__':
    ...
    user_profiles,\
    user_reviews,\
    _ = get_users_businesses_wordmap(model_file)
    hasher = Hasher(bins_num=27351, a=9679, b=23143)
    uniq_pairs = {}
    number_of_hash = 200
    bands = 100
    bands_r = 2
    test_users,test_bisnesses = get_test_data(test_file=test_file)
    user_profiles = remove_useless_entires(user_profiles,test_users)
    business_profiles = remove_useless_entires(business_profiles,test_bisnesses)
    merged_profiles = {**user_profiles,**business_profiles}

    predictions = spark_context.parallelize(merged_profiles.items(),numSlices=50)\
                                    .flatMap(lambda x: [(band_hash, x[0]) for band_hash in map_bands(x[1], bands, bands_r)])\
                                    .groupByKey()\
                                    .filter(lambda x: len(x[1]) > 1)\
                                    .map(lambda x: (x[0],x[1]))\
                                    .flatMap(lambda x: [(*sorted((id1,id2)),cosineSim(id1,id2,merged_profiles)) \
                                        for id1,id2 in combinations(x[1],2) if is_uniq_pair(uniq_pairs,id1,id2) and cosineSim(id1,id2,merged_profiles)>=0.01]).collect()
